...
// when no other goroutine is reading from the connection, and the caller must
...
func (c *Conn) WaitForClientActivity(ctx context.Context) error {
	...
	// deadline in the past, which wakes the blocked Peek below. The helper
	// goroutine is always joined (via deadlineDone) before we reset the
	// deadline, so there is no window in which it could set a stale past
	// deadline after the reset.
	//
	// Note: when the connection is wrapped in netutil.ConnWithTimeouts, setting
	// a deadline here does not disable that wrapper's managed idle timeout (its
	// methods have value receivers and re-arm the deadline on every Read); the
	// reset below matters for the unwrapped case, where no managed timeout
	// exists.
	...
	// If we were asked to stop, ignore whatever the Peek observed: it may have
	// been our own past deadline, or a real disconnect that raced with the
	// cancellation. Either way the caller is no longer interested.
	if ctx.Err() != nil {
		return nil
	}
	if err != nil {
